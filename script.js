...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Recuperar o tema salvo ou definir padrão
    let savedTheme = localStorage.getItem("theme") || "tema_padrão.css";
    themeStyle.href = savedTheme;
    themeSelect.value = savedTheme;

    // Definir o estado do checkbox com base no tema salvo
    darkModeToggle.checked = savedTheme.includes("escuro");

    // Evento para mudança via <select>
    themeSelect.addEventListener("change", function () {
        let selectedTheme = this.value;
        themeStyle.href = selectedTheme;
        localStorage.setItem("theme", selectedTheme);
        darkModeToggle.checked = selectedTheme.includes("escuro");
    });

    // Evento para ativar/desativar modo escuro
    darkModeToggle.addEventListener("change", function () {
        let currentTheme = themeSelect.value;
        let newTheme;

        if (this.checked) {
            if (currentTheme === "tema_padrão.css") {
                newTheme = "tema_escuro_padrão.css";
            } else if (currentTheme === "tema_elegante.css") {
                newTheme = "tema_escuro_elegante.css";
            } else {
                newTheme = currentTheme; // Se já estiver escuro, mantém
            }
        } else {
            if (currentTheme === "tema_escuro_padrão.css") {
                newTheme = "tema_padrão.css";
            } else if (currentTheme === "tema_escuro_elegante.css") {
                newTheme = "tema_elegante.css";
            } else {
                newTheme = currentTheme; // Se já estiver claro, mantém
            }
        }
